...
(function () {
  ...
  function bm(a, b) {
    function f() {
      ...
      (c.lastIndex = 0),
        c.test(e) ||
          ((e = n(e + " " + a)), d ? (b.baseVal = e) : (this.className = e));
    }
          ...
          return (
            a.each(function (h, i, j) {
              ...
              function q(a) {
                ...
                if (c >= 1)
                  return r(), (bz = b), e.end.call(l, h, i), (bz = 0), 1;
              }
  ...
  function bI(a) {
    ...
    b[0] * c[1] < c[0] * b[1] &&
      ((b[0] *= -1), (b[1] *= -1), (d *= -1), (e *= -1)),
      (this.rotate =
        (d ? Math.atan2(b[1], b[0]) : Math.atan2(-c[0], c[1])) * bM),
  ...
  function bX(a, b) {
    return d3.format(
      ",." +
        Math.max(0, -Math.floor(Math.log(bV(a, b)[2]) / Math.LN10 + 0.01)) +
        "f"
    );
  }
  ...
  function cA(a, b) {
    return a.length < 3
      ? cw(a)
      : a[0] +
          cC((a.push(a[0]), a), cD([a[a.length - 2]].concat(a, [a[1]]), b));
  }
  ...
  function cE(a) {
    ...
    cM(i, g, h);
    while (++b < c)
      (d = a[b]), g.shift(), g.push(d[0]), h.shift(), h.push(d[1]), cM(i, g, h);
    b = -1;
    while (++b < 2)
      g.shift(), g.push(d[0]), h.shift(), h.push(d[1]), cM(i, g, h);
    return i.join("");
  }
  ...
  function cF(a) {
    ...
    while (++c < d)
      (e = a[c]), f.shift(), f.push(e[0]), g.shift(), g.push(e[1]), cM(b, f, g);
    return b.join("");
  }
  ...
  function dy() {
    d3.event.keyCode == 32 &&
      dr == 2 &&
      ((du[0] += dq[1][0]), (du[1] += dq[1][1]), (dr = 0), O());
  }
  ...
  function dz() {
    if (du) {
      ...
      dr ||
        (d3.event.altKey
          ? (dt ||
              (dt = [(dq[0][0] + dq[1][0]) / 2, (dq[0][1] + dq[1][1]) / 2]),
            (du[0] = dq[+(a[0] < dt[0])][0]),
            (du[1] = dq[+(a[1] < dt[1])][1]))
          : (dt = null)),
    ...
    function j(j) {
      j.each(function (k, l, m) {
        var n = d3.select(this),
          ...
          q =
            h == null ? (a.tickFormat ? a.tickFormat.apply(a, g) : String) : h,
          r = dj(a, p, i),
    ...
    (d3.behavior.drag = function () {
      ...
      function d() {
        (dD = a),
          ...
          (dI = dL()),
          b
            ? ((dH = b.apply(dF, dG)), (dH = [dH.x - dI[0], dH.y - dI[1]]))
            : (dH = [0, 0]),
          (dJ = 0);
      }
  ...
  d3.behavior.zoom = function () {
    ...
    function i() {
      ...
      b.length === 1 &&
        d - dT < 300 &&
        eg(1 + Math.floor(a[2]), (c = b[0]), dS[c.identifier]),
        (dT = d);
    }
    var a = [0, 0, 0],
      b = d3.dispatch("zoom"),
...
(function () {
  ...
  (d3.geom.hull = function (a) {
    ...
    f.push(i);
    for (g = 0, h = 0; g < 2; ++h)
      e[h].index !== -1 && (f.push(e[h].index), g++);
    q = f.length;
    for (; h < c; ++h) {
        ...
        for (var m = 3; m < f; m++) {
          ...
          if (n == 1)
            for (k = g._pack_prev; k !== j._pack_prev; k = k._pack_prev, p++)
              if (I(k, i)) break;
          n
            ? (o < p || (o == p && h.r < g.r) ? H(g, (h = j)) : H((g = k), h),
              m--)
            : (G(g, i), (h = i), l(i));
        }
      }
  ...
  return {
    ...
    init: function () {
      ...
      for (var i = 0; i < this.data.length; i++) {
        ...
        if (n.positions.total) {
          out.x =
            n.positions.total.x + (n.positions.total.x - that.width / 2) * 0.5;
          out.y = n.positions.total.y + (n.positions.total.y - 150) * 0.5;
        }
        if (
          n[this.currentYearDataColumn] > 0 !==
      ...
      this.circle
        ...
        .on("mouseover", function (d, i) {
          ...
          d3.select("#nytg-tooltip .nytg-discretion").text(
            that.discretionFormat(d.discretion)
          );
          d3.select("#nytg-tooltip .nytg-department").text(d.group);
          d3.select("#nytg-tooltip .nytg-value").html(
            "$" + that.bigFormat(d.value)
          );
    ...
    buoyancy: function (alpha) {
      ...
      return function (d) {
        ...
        var targetY =
          that.centerY - (d.changeCategory / 3) * that.boundingRadius;
        d.y =
          d.y +
          (targetY - d.y) * that.defaultGravity * alpha * alpha * alpha * 100;
      };
    },
    ...
    mandatorySort: function (alpha) {
      ...
      return function (d) {
        ...
        if (d.discretion === that.DISCRETIONARY) {
          ...
          d.discretion === that.MANDATORY ||
          d.discretion === that.NET_INTEREST
        ) {
          targetX = 400;
        } else {
          targetX = 900;
    ...
    discretionarySort: function (alpha) {
      ...
      return function (d) {
        ...
        if (d.discretion === "Discretionary") {
          ...
          d.discretion === "Mandatory" ||
          d.discretion === "Net interest"
        ) {
        ...
        d.y =
          d.y + (targetY - d.y) * Math.sin(Math.PI * (1 - alpha * 10)) * 0.2;
        d.x =
          d.x + (targetX - d.x) * Math.sin(Math.PI * (1 - alpha * 10)) * 0.1;
      };
    },
